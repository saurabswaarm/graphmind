...
async def get_graph(
    ...
    entity_type: Optional[str] = Query(None, description="Filter by entity type (when scope=by_type)"),
    ...
    if scope == GraphScope.ALL:
        ...
        if nodes:
            ...
            if relationship_type:
                relationship_query = relationship_query.where(Relationship.type == relationship_type)
            
            relationship_query = relationship_query.limit(limit_edges + 1)  # +1 to check if truncated
        ...
        for current_depth in range(depth):
            ...
            for entity_id in frontier:
                ...
                out_rel_query = select(Relationship).where(Relationship.source_entity_id == entity_id)
                if relationship_type:
                ...
                in_rel_query = select(Relationship).where(Relationship.target_entity_id == entity_id)
                if relationship_type:
